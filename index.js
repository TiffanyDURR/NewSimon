...
function tourIA() {
  ...
  notesCountElement.innerHTML = `Nombre de notes a trouver: ${IAData.length}`;

  const remainingNotesElement = document.querySelector("#remainingNotes");
  remainingNotesElement.innerHTML = `Nombre de notes restantes: ${IAData.length}`;
...
function tourDuJoueur() {
  ...
  jurons.forEach((juron) => {
    ...
    juronID.addEventListener("click", (e) => {
      ...
      // Mise à jour du nombre de notes restantes
      const remainingNotesElement = document.querySelector("#remainingNotes");
      remainingNotesElement.innerHTML = `Nombre de notes restantes: ${IAData.length - playerData.length}`;

      // Vérification de la note jouée
      if (playerData[playerData.length - 1] !== IAData[playerData.length - 1]) {
        score = 0;
      ...
      // Vérification automatique de la séquence
      if (playerData.length === IAData.length) {
...
setTimeout(function () {juronID.classList.remove("animation");},180);
}
